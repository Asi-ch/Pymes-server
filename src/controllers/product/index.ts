...
export class ProductsController {
    ...
    switch (method) {
      case "create": {
        ...
        return [
          ...
          check("productVariation.*.quantity").not().isEmpty(),

          check("productVariation.*.cost").not().isEmpty(),
          check("productVariation.*.quantity").not().isEmpty(),
        ];
      }
    ...
    try {
      ...
      if (req.body.productVariation && product) {
        ...
        if (variations.length > 0) {
          await Promise.all(
            variations.map(async (singleVariation: any) => {
        ...
        if (product) {
          ...
          return res.status(200).json({
            success: true,
            message:
              "Product was created successfully. But no product variations were found",
            data: prod,
          });
        } else {
          return res.status(200).json({
  ...
  public async updateProductVariation(req: Request, res: Response) {
    try {
      ...
      if (productVariation) {
        ...
        return res
          .status(404)
          .json({ success: false, message: "Product variation not found " });
      }
    } catch (error) {
      console.log("Error ==> ", error);
  ...
  public async getAllProductVariants(req: Request, res: Response) {
    try {
      const allVariants = await ProductVariation.findAll({
      ...
      if (allVariants.length > 0) {
        ...
        return res
          .status(404)
          .json({ success: false, message: "Product Variants not found" });
      }
    } catch (error) {
      return res.status(500).json({ success: false, error });
    }
  ...
  public async addProductVariant(req: Request, res: Response) {
    try {
      ...
      if (productVariant) {
        ...
        return res
          .status(400)
          .json({ success: false, message: "Variation not added" });
      }
    } catch (error) {
      return res.status(500).json({ success: false, error });
    }
